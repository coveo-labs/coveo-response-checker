...
backgroundPageConnection.onMessage.addListener(function (message) {
  ...
  if (message.all) {
    ...
    if (message.all[i].statusCode) {
      ...
      line += `<span class='url ${statusok}'><a href='${message.all[i].request.url}' target='_blank'>${message.all[i].request.url}</a>${status}</span>`;
    ...
    //line += `<span class=code style='cursor:pointer'" id=${id}>Data sent(click to show):<pre class='mycode' id=${idc}>${JSON.stringify(message.request.data,null,2)}</pre></span>`;
    line += `<span class='url ${statusok}'><a href='${message.request.url}' target='_blank'>${message.request.url}</a>${status}</span>`;
